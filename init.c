...
void* POSIX_Init(void *argument)
{
 ...
 func[0] = sensor1; func[1] = sensor2; func[2] = sensor3;
 status = pthread_redundancy_init(&red, attr, func, NULL);
 if(status !=0 )
     returncode("pthread_redundancy_init init.c",status);


 diff = pthread_redundancy_vote(&red, &my_result);
 if(status != 0)
     returncode("pthread_redundancy_vote init.c",status);
